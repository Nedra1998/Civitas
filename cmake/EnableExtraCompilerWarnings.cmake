...
# globally_enable_extra_compiler_warnings() - to modify CMAKE_CXX_FLAGS, etc to
# change for all targets declared after the command, instead of per-command
#
# Original Author: 2010 Ryan Pavlik <rpavlik@iastate.edu> <abiryan@ryand.net>
# http://academic.cleardefinition.com Iowa State University - HCI Graduate
# Program/VRAC
#
# Additional Author: 2013 Kevin M. Godby <kevin@godby.org>
# http://kevin.godby.org/ Iowa State University - HCI Graduate Program/VRAC
#
# Copyright 2009-2010, Iowa State University Distributed under the Boost
# Software License, Version 1.0. (See accompanying file LICENSE_1_0.txt or copy
# at http://www.boost.org/LICENSE_1_0.txt) SPDX-License-Identifier: BSL-1.0
...
# @hidden @see `_enable_extra_compiler_warnings_flags`
...
# @hidden @see `enable_extra_compiler_warnings` @see
# `globally_enable_extra_compiler_warnings`
macro(_enable_extra_compiler_warnings_flags)
  set(_flags)
  if(MSVC)
    option(COMPILER_WARNINGS_EXTREME
           "Use compiler warnings that are probably overkill." off)
...
# Adds additional compiler warning flags that the compiler supports.
#
# This will detect which compiler warning flags are supported by the current
# compiler and add any additional flags that are currently missing from the
# existing `COMPILER_FLAGS` for the provided target.
#
# @arg _target The target to add the warning flags to.
...
# This will detect which compiler warning flags are supported by the current
# compiler and add any additional flags that are currently missing from the
# existing `COMPILER_FLAGS` for the global `CMAKE_C_FLAGS` and
# `CMAKE_CXX_FLAGS`.
function(globally_enable_extra_compiler_warnings)
  _enable_extra_compiler_warnings_flags()
  set(CMAKE_C_FLAGS
      "${CMAKE_C_FLAGS} ${_flags}"
      PARENT_SCOPE)
  set(CMAKE_CXX_FLAGS
      "${CMAKE_CXX_FLAGS} ${_flags}"
      PARENT_SCOPE)
endfunction()
